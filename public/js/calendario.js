    ...
    function getCalendar(perfil, div)
    {
        ...
        let calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventClick: function(info) {
                if(perfil == 'Tutor'){
                    // win.location.href=`/sisfis/view/home/dashboard.php?menuop=agendamento&idAgendar=${info.event.id}&start=${info.event.start}`;
                }else if(perfil == 'Discente'){
                    win.location.href=`/sisfis/view/home/dashboard.php?menuop=horario&idAgendar=${info.event.id}&start=${info.event.start}`;
                }else if(perfil == 'Tutor1'){
                    win.location.href=`/sisfis/view/home/dashboard.php?menuop=horario&idAgendar=${info.event.id}&start=${info.event.start}`;
                }else if(perfil == 'Discente1'){
                    win.location.href=`/sisfis/view/home/dashboard.php?menuop=horario&idAgendar=${info.event.id}&start=${info.event.start}`;
